...
def change_dection(video):
    ...
    """
    gmm_foreground = change_dection_gmm(video)
    # clustering_foreground = change_dection_clustering(video)


def change_dection_gmm(video):
    """
    Modeling each pixel as a mixture of Gaussians and using an on-line approximation to
    update the model. The Gaussian distributions of the adaptive mixture model are
    evaluated to determine which are most likely to result from a background process.

    :param video: Video to check
    :return: Array of frames with indication of foreground objects
    """
...
def change_dection_clustering(video):
    """
    Clustering-based algorithm - an algorithm that does not assume a background model,
    but learns the past history using an unsupervised incremental clustering algorithm.
    Basically, partitioning the frame into 4X4 blocks, for each block we have a set of clusters.
    Using the Manhattan distance we find the difference of a new frame from the past ones.
    Using a predefined threshold we decide on changes, and finally updating the clusters
    after each frame.
...
# def median_change_dection(video, k1, k2, background_th):
#     """
#     Decide if there was a significant change in the video or not.
#     """
#     v_foreground = np.zeros(video.shape)
